...
// POST /api/coach — one focused agent with four expert lenses.
// It uses only the user's Life OS context; no web search or invented benchmarks.
app.post("/api/coach", async (req, res) => {
  const { question, state, preferredLens = "auto" } = req.body || {};
  ...
  const prompt = `Bạn là Life OS Coach — một AI agent duy nhất có đúng 4 kỹ năng chuyên gia:
...
4) CAREER 30M+: định vị năng lực, CV/portfolio, chiến lược ứng tuyển, phỏng vấn, thương lượng lương net và theo dõi pipeline tuyển dụng.

NGUYÊN TẮC BẮT BUỘC:
- Chỉ dùng dữ liệu Life OS được cung cấp bên dưới. Không tìm kiếm web, không viện dẫn xu hướng chung, không bịa dữ liệu.
- LENS NGƯỜI DÙNG CHỌN: ${preferredLens}. Nếu khác "auto", phải tư vấn đúng lens đó. Chỉ dùng cross_goal khi người dùng chọn auto và câu hỏi thực sự liên quan nhiều mục tiêu.
    ...
    const response = await generateWithRetry({
      ...
      config: {
        ...
        responseSchema: {
          type: Type.OBJECT,
          properties: {
            expertLens: { type: Type.STRING, description: "fund_backtest, b2b_marketing, career, health_beauty hoặc cross_goal" },
