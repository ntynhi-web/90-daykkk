...
export default function TodayView({ state, onChangeState, onOpenProgress }: TodayViewProps) {
  ...
  return (
    <div id="today-dashboard-view" className="space-y-8">
      ...
      <section className="rounded-[24px] border-2 border-emerald-300 bg-emerald-50/60 p-5 shadow-sm">
        <div className="flex items-center justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-[0.16em] text-emerald-700">02 · Đã sống & đã làm</p><h2 className="mt-1 text-xl font-black text-slate-950">Dòng thời gian hôm nay</h2><p className="mt-1 text-xs text-slate-500">Ghi việc thực tế theo giờ; app dùng dữ liệu này để vẽ biểu đồ tiến bộ.</p></div><span className="rounded-full bg-white px-3 py-1 text-[10px] font-black text-emerald-700">{todayActivities.length} ghi nhận</span></div>
        {todayActivities.length > 0 ? <div className="mt-4 flex gap-3 overflow-x-auto pb-1">{todayActivities.map(activity => { const goal = state.goals.find(item => item.id === activity.goalId); const displayTime = activity.startTime || new Date(activity.createdTimestamp).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Ho_Chi_Minh' }); return <article key={activity.id} className="min-w-[220px] rounded-2xl border border-emerald-100 bg-white p-4"><div className="flex items-center justify-between gap-2"><span className="font-mono text-xs font-black text-emerald-700">{displayTime}{activity.endTime ? `–${activity.endTime}` : ''}</span>{activity.durationMinutes ? <span className="text-[9px] font-bold text-slate-400">{activity.durationMinutes} phút</span> : null}</div><p className="mt-2 text-sm font-black text-slate-900">{activity.activity}</p><p className="mt-1 text-[10px] text-slate-500">{goal?.name || 'Cuộc sống hằng ngày'}</p>{typeof activity.output?.weightKg === 'number' && <p className="mt-2 text-xs font-black text-rose-600">{activity.output.weightKg} kg</p>}</article>; })}</div> : <div className="mt-4 flex flex-col items-center justify-between gap-3 rounded-2xl border border-dashed border-emerald-300 bg-white/80 p-4 text-center sm:flex-row sm:text-left"><div><p className="text-sm font-black text-slate-800">Chưa có hành động nào được lưu hôm nay</p><p className="mt-1 text-xs text-slate-500">Nhập giờ chạy bộ, cân nặng hoặc việc đã làm để bắt đầu biểu đồ.</p></div><button type="button" onClick={() => document.getElementById('section-quick-input')?.scrollIntoView({ behavior: 'smooth' })} className="shrink-0 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white">Ghi hoạt động đầu tiên</button></div>}
      </section>
