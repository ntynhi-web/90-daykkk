...
export default function ProgressView({ state, onChangeState }: ProgressViewProps) {
  ...
  const milestoneProgressLogs = state.milestoneProgressLogs || [];
  const latestMilestoneLog = (milestoneId: string) => milestoneProgressLogs
    .filter(log => log.milestoneId === milestoneId)
    .sort((a, b) => b.createdTimestamp - a.createdTimestamp)[0];
  const currentMilestoneSnapshots = activeGoals.map(goal => {
    const milestone = goal.milestones.find(item => item.id === goal.currentMilestoneId) || goal.milestones.find(item => !item.achieved && item.status !== 'skipped');
    const latest = milestone ? latestMilestoneLog(milestone.id) : undefined;
    return { goal, milestone, latest, progress: milestone?.achieved ? 100 : latest?.progress || 0 };
  }).filter(item => item.milestone);
  const last7Days = [...Array(7)].map((_, index) => new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Ho_Chi_Minh", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(Date.now() - (6 - index) * 86_400_000)));
  const sevenDayActivity = last7Days.map(date => ({ date, count: state.activities.filter(activity => activity.date === date).length }));
  const sevenDayMax = Math.max(1, ...sevenDayActivity.map(item => item.count));
      ...
      <section className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div className="space-y-1">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="font-display font-extrabold text-3xl text-[#0b0f19] tracking-tight">Kết quả chu kỳ</h2>
            <span className="rounded-full border border-rose-200 bg-rose-50 px-3 py-1 text-xs font-black text-rose-700">Hạn chót · {formatDisplayDate(state.endDate)}</span>
          </div>
          <p className="text-sm text-slate-500 max-w-xl">
            Theo dõi tiến độ và kết quả thực tế đến ngày {formatDisplayDate(state.endDate)}.
          </p>
        </div>
          ...
          >
            ...
            <section className="grid gap-4 xl:grid-cols-[1.6fr_0.8fr]">
              <div className="rounded-[24px] border border-indigo-100 bg-white p-5 shadow-sm">
                <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[10px] font-black uppercase tracking-wider text-indigo-600">Tiến bộ theo cột mốc</p><h3 className="mt-1 text-lg font-black text-slate-950">Không cần đợi hoàn tất mới được ghi nhận</h3></div><span className="rounded-full bg-indigo-50 px-3 py-1 text-[10px] font-black text-indigo-700">{milestoneProgressLogs.length} lần cập nhật</span></div>
                <div className="mt-4 grid gap-3 md:grid-cols-2">{currentMilestoneSnapshots.map(({ goal, milestone, latest, progress }) => <article key={`${goal.id}:${milestone?.id}`} className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="truncate text-[10px] font-black uppercase tracking-wide text-slate-400">{goal.name}</p><p className="mt-1 text-sm font-black text-slate-900">{milestone?.title}</p></div><span className="font-mono text-lg font-black text-indigo-700">{progress}%</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${progress}%` }} /></div><p className="mt-2 min-h-8 text-[10px] leading-relaxed text-slate-500">{latest?.note || 'Chưa có cập nhật. Ghi bước nhỏ đầu tiên trong Lộ trình.'}</p>{latest && <p className="mt-1 text-[9px] font-bold text-slate-400">{formatDisplayDate(latest.date)}{latest.time ? ` · ${latest.time}` : ''}</p>}</article>)}</div>
              </div>
              <div className="rounded-[24px] border border-emerald-100 bg-white p-5 shadow-sm"><p className="text-[10px] font-black uppercase tracking-wider text-emerald-700">Nhịp hành động 7 ngày</p><h3 className="mt-1 text-lg font-black text-slate-950">Hoạt động lên xuống</h3><div className="mt-5 flex h-32 items-end gap-2">{sevenDayActivity.map(item => <div key={item.date} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-2"><span className="text-[9px] font-black text-slate-500">{item.count}</span><div className="w-full rounded-t-lg bg-emerald-500/80 transition-all" style={{ height: `${Math.max(item.count ? 12 : 3, item.count / sevenDayMax * 90)}%` }} /><span className="text-[8px] font-bold text-slate-400">{item.date.slice(8)}</span></div>)}</div><p className="mt-3 text-[10px] leading-relaxed text-slate-500">Số hành động thực tế đã ghi mỗi ngày, không phải số task được lên kế hoạch.</p></div>
            </section>
