...
export default function PlanHub({ state, onChangeState }: { state: AppState; onChangeState: (state: AppState) => void }) {
  ...
  useEffect(() => {
    if (schedulePage >= schedulePageCount) setSchedulePage(Math.max(0, schedulePageCount - 1));
  ...
  const exportTemplate = () => {
    const plan = {
      ...
      plan: {
        ...
        goals: state.goals.map(goal => ({ ...goal, currentProgress: 0, status: "active", milestones: goal.milestones.map(step => ({ ...step, achieved: false, currentValue: "0", completedAt: null, status: step.parallel || step.order === 0 ? "active" : "locked" })) })),
  ...
  const applyPlan = (plan: any) => {
    ...
    const hasTrackingData = Boolean(state.activities?.length || state.routineLogs?.length || state.milestoneProgressLogs?.length || state.weeklyReviews?.length);
    if (hasTrackingData && !window.confirm("Import kế hoạch mới sẽ xóa dữ liệu tiến độ/check-in hiện tại để tránh liên kết sai mục tiêu. Bạn có muốn tiếp tục?")) {
      throw new Error("Đã hủy import; dữ liệu hiện tại được giữ nguyên.");
    }
    onChangeState({
      ...state,
      ...validated,
  ...
  return (
    <section className="rounded-[28px] border border-indigo-200 bg-white p-5 shadow-sm md:p-7">
      ...
      {tab === "goals" && <div className="mt-5 space-y-3">{state.goals.length === 0 && <p className="rounded-2xl border border-dashed p-6 text-center text-xs text-slate-500">Chưa có mục tiêu. Hãy import template hoặc tạo mục tiêu ở Trang Mục tiêu.</p>}{state.goals.map(goal => <article key={goal.id} className="rounded-2xl border border-slate-200 p-4"><input maxLength={160} value={goal.name} onChange={event => patchGoal(goal.id, { name: event.target.value })} className="w-full rounded-lg border border-slate-200 px-3 py-2 text-sm font-black" /><textarea maxLength={MAX_TEXT_LENGTH} value={goal.description || ""} onChange={event => patchGoal(goal.id, { description: event.target.value })} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><div className="mt-3 flex gap-2 overflow-x-auto">{goal.milestones.map(step => <div key={step.id} className="min-w-[210px] rounded-xl bg-slate-50 p-3"><input maxLength={160} value={step.title} onChange={event => patchGoal(goal.id, { milestones: goal.milestones.map(item => item.id === step.id ? { ...item, title: event.target.value } : item) })} className="w-full rounded-lg border px-2 py-1.5 text-xs font-bold" /><input maxLength={500} value={step.targetValue} onChange={event => patchGoal(goal.id, { milestones: goal.milestones.map(item => item.id === step.id ? { ...item, targetValue: event.target.value } : item) })} className="mt-2 w-full rounded-lg border px-2 py-1.5 text-[11px]" /><input type="date" value={step.dueDate} min={state.startDate} max={state.endDate} onChange={event => patchGoal(goal.id, { milestones: goal.milestones.map(item => item.id === step.id ? { ...item, dueDate: event.target.value } : item) })} className="mt-2 w-full rounded-lg border px-2 py-1.5 text-[11px]" /></div>)}</div></article>)}</div>}
      {tab === "routines" && <div className="mt-5 grid gap-3 lg:grid-cols-2">{state.routines.map(routine => <article key={routine.id} className="rounded-2xl border p-4"><input maxLength={160} value={routine.name} onChange={event => onChangeState({ ...state, routines: state.routines.map(item => item.id === routine.id ? { ...item, name: event.target.value } : item) })} className="w-full rounded-lg border px-2 py-1.5 text-xs font-black" />{(["frequency","minimumDay","target"] as const).map(field => <input key={field} maxLength={500} value={routine[field]} onChange={event => onChangeState({ ...state, routines: state.routines.map(item => item.id === routine.id ? { ...item, [field]: event.target.value } : item) })} className="mt-2 w-full rounded-lg border px-2 py-1.5 text-[11px]" />)}</article>)}</div>}
      {tab === "schedule" && <div className="mt-5">{scheduleItems.length === 0 && <p className="rounded-2xl border border-dashed p-6 text-center text-xs text-slate-500">Chưa có lịch. Import template hoặc thêm việc trong Calendar.</p>}<div className="max-h-[520px] space-y-2 overflow-y-auto">{visibleScheduleItems.map(item => <article key={item.id} className="grid gap-2 rounded-xl border p-3 md:grid-cols-[130px_90px_90px_minmax(0,1fr)]">{(["date","startTime","endTime","title"] as const).map(field => <input key={field} maxLength={field === "title" ? 160 : undefined} type={field === "date" ? "date" : field.includes("Time") ? "time" : "text"} value={item[field]} min={field === "date" ? state.startDate : undefined} max={field === "date" ? state.endDate : undefined} onChange={event => onChangeState({ ...state, scheduleItems: state.scheduleItems.map(row => row.id === item.id ? { ...row, [field]: event.target.value } : row) })} className="rounded-lg border px-2 py-1.5 text-[11px]" />)}</article>)}</div>{schedulePageCount > 1 && <div className="mt-3 flex items-center justify-between"><button disabled={schedulePage === 0} onClick={() => setSchedulePage(page => Math.max(0, page - 1))} className="rounded-lg border px-3 py-2 text-xs font-bold disabled:opacity-40">Trang trước</button><span className="text-xs font-semibold text-slate-500">{schedulePage + 1}/{schedulePageCount} · {scheduleItems.length} lịch</span><button disabled={schedulePage >= schedulePageCount - 1} onClick={() => setSchedulePage(page => Math.min(schedulePageCount - 1, page + 1))} className="rounded-lg border px-3 py-2 text-xs font-bold disabled:opacity-40">Trang sau</button></div>}</div>}
