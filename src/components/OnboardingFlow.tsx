          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="space-y-2">{state.routines.map(routine => <div key={routine.id} className={`grid gap-3 rounded-2xl border p-3 sm:grid-cols-[auto_160px_minmax(0,1fr)] sm:items-center ${routine.active === false ? 'border-slate-100 bg-slate-50 opacity-60' : 'border-slate-200'}`}><button type="button" aria-pressed={routine.active !== false} onClick={() => onChangeState({ ...state, routines: state.routines.map(item => item.id === routine.id ? { ...item, active: item.active === false } : item) })} className={`h-6 w-10 rounded-full p-0.5 transition ${routine.active !== false ? 'bg-emerald-500' : 'bg-slate-300'}`}><span className={`block h-5 w-5 rounded-full bg-white shadow transition ${routine.active !== false ? 'translate-x-4' : ''}`} /></button><span className="text-xs font-bold text-slate-800">{routine.name}</span><input disabled={routine.active === false} value={routine.minimumDay} onChange={event => onChangeState({ ...state, routines: state.routines.map(item => item.id === routine.id ? { ...item, minimumDay: event.target.value } : item) })} className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs outline-none focus:border-indigo-400 disabled:cursor-not-allowed" /></div>)}</div>
              </div>
            </div>
          )}
